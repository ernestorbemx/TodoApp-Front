...
describe("test TodoForm component", () => {
    ...
    it("should render", () => {
        const wrapper = render(
            <Modal isOpen>
                <TodoForm label="Test form" loading={false} onClose={() => { }} onChange={() => { }} />
            </Modal>)
    ...
    it("should call onClose when button click", async () => {
        ...
        const wrapper = render(
            <Modal isOpen>
                <TodoForm label="Test form" loading={false} onClose={onCloseFn} onChange={() => { }} />
            </Modal>)
    ...
    it("should not call onChange when form is not correct", async () => {
        ...
        const wrapper = render(
            <Modal isOpen>
                <TodoForm label="Test form" loading={false} onClose={() => {}} onChange={onChangeFn} />
            </Modal>)
    ...
    it("should call onChange when form submit", async () => {
        ...
        expect(onChangeFn).toBeCalledTimes(0)
        expect(wrapper.findByTestId("todo-form-submit")).toBeDefined()
        await userEvent.type(await wrapper.findByTestId("todo-form-text"), "Todo text")
